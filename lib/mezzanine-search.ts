 ...
 * Mezzanine's full production catalog for shows in neither our scored catalog
 * nor the diary-only catalog.
 *
 * Straight port of the web's src/lib/mezzanine-search.ts against the same
 * deployed function, so the app's per-row "Find the production I saw" offers
 * the same candidates the website does.
 */
import { getSupabaseClient } from './supabase';

/** One ranked search result — already shaped to become a user_show_stubs row
 *  (id matches the web's buildDiarySlug() output). */
...
async function invokeSearch(body: { query: string } | { mezzShowId: string }): Promise<MezzanineCandidate[]> {
  const supabase = getSupabaseClient();
  if (!supabase) throw new Error(MEZZANINE_SEARCH_ERROR_COPY.unauthorized);

  const { data, error } = await supabase.functions.invoke<MezzanineSearchResponse>('mezzanine-search', { body });
...
export function searchMezzanineCatalog(query: string): Promise<MezzanineCandidate[]> {
  return invokeSearch({ query });
}

/** Resolve a single Mezzanine Show by its known objectId — exact, no fuzzy
 *  title search, used when the export already carries entry.show.id. */
export function resolveMezzanineShow(mezzShowId: string): Promise<MezzanineCandidate[]> {
  return invokeSearch({ mezzShowId });
}
