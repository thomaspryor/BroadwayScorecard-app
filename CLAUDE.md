...
2. **Venue: the Artifact, NOT Claude Design.** Confirmed 2026-07-22: DesignSync/`claude.ai/design` projects are **structurally inaccessible to the owner** — the DesignSync tool authorization is bound to an account/org the owner's own claude.ai login cannot see. Verified with a real click-through on TWO separate DesignSync projects (`469cbecf-...` and the dedicated `d21b75cc-...` proposals project) — both returned "You don't have access to this project," even after separating proposals out of the design-system project into their own. This is not a "wrong project" or "wrong category" issue and not fixable by re-auth from this side — do not keep retrying DesignSync as the delivery venue. **Deliver design proposals as a Claude Artifact** (Artifact tool), same content/images, published to a stable URL and republished (same URL) on revision. Current one: `https://claude.ai/code/artifact/708007ba-4153-4d88-bb06-581e3388e8c9`. If ever revisiting DesignSync as a venue, the bar is an owner click-through confirming access BEFORE building anything there — API `canEdit:true` is not evidence.
