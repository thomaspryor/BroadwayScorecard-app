 ...
 *   BSC_WEB_REPO=/path/to/web node scripts/…    # override the source repo
...
const WEB_REPO = process.env.BSC_WEB_REPO || path.join(os.homedir(), 'Broadwayscore');
...
function readSource(file) {
  ...
  if (!fs.existsSync(p)) {
    throw new Error(`Missing source file ${p}. Set BSC_WEB_REPO to the Broadwayscore checkout.`);
  }
...
function buildVendorSet() {
  ...
  for (const [dest, spec] of Object.entries(DERIVED)) {
    if (!fs.existsSync(spec.source)) {
      throw new Error(`Missing derived source ${spec.source}. Set BSC_WEB_REPO.`);
    }
...
function write(set) {
  ...
  const manifest = {
    _comment:
      'Generated by scripts/vendor-stats-lib.js. `sha256` guards the vendored copy (checked in CI without the web repo); `sourceSha256` guards the web-repo original (checked when BSC_WEB_REPO is present).',
...
module.exports = { buildVendorSet, check, sha256, VERBATIM, DERIVED, DEST_DIR, MANIFEST_PATH, SRC_DIR, WEB_REPO };
